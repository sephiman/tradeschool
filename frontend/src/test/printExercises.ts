...
import { CONTENT_DIR, exerciseKey, manifestModules, type Locale } from "@/test/courseContent";
...
/** A small stable hash, so every choice below is a function of the KEY — no clock, no randomness. */
...
/** The printed seed's stand-in — same contract (a pure function of the exercise KEY), different hash. */
...
function quiz(key: string, raw: RawConfig, locale: Locale): Omit<PrintExercise, "id" | "number" | "type" | "isChart" | "seed"> {
  const variants = raw.variants ?? [];
  const variant = variants[digest(key) % variants.length];
  ...
  if (kind === "ordering") {
    const items = shuffled(variant.items ?? [], digest(key));
  ...
  if (kind === "matching") {
    ...
    const lefts = shuffled(pairs.map((p, i) => ({ p, i })), digest(`l${key}`));
    const rights = shuffled(pairs.map((p, i) => ({ p, i })), digest(`r${key}`));
  ...
  const options = shuffled(variant.options ?? [], digest(key));
  return {
    prompt,
...
function calculation(key: string, raw: RawConfig, locale: Locale): Omit<PrintExercise, "id" | "number" | "type" | "isChart" | "seed"> {
  // The real generator samples the params and formats them into the prompt; a fixed sample is enough
  // to print, and keeps the fixture a pure function of the key.
  ...
  const base = 1 + (digest(key) % 900);
  const options: OptionView[] = [0, 1, 2, 3].map((i) => ({
    id: `o${i}`,
    value: (base * (1 + i / 10)).toFixed(2),
  }));
  const correct = options[digest(`c${key}`) % options.length];
  return {
    prompt,
...
function series(key: string): NonNullable<AttemptPayload["series"]> {
  const time: number[] = [];
  const open: number[] = [];
  const high: number[] = [];
  const low: number[] = [];
  const close: number[] = [];
  const volume: number[] = [];
  let price = 100 + (digest(key) % 50);
  for (let i = 0; i < BARS; i++) {
    const step = ((digest(`${key}:${i}`) % 200) - 100) / 100;
    ...
    volume.push(1000 + (digest(`v${key}:${i}`) % 500));
  }
  return { time, open, high, low, close, volume };
}

function chart(key: string, raw: RawConfig, locale: Locale): Omit<PrintExercise, "id" | "number" | "type" | "isChart" | "seed"> {
  const choices = raw.choices ?? ["none"];
  const label = choices[digest(key) % choices.length];
  const bars = series(key);
  const rsi = bars.close.map((_, i) => 30 + (digest(`r${key}:${i}`) % 40));
...
export function printExerciseFromContent(
  ...
  const body = isChart
    ? chart(key, raw, locale)
    : type === "calculation"
      ? calculation(key, raw, locale)
      : quiz(key, raw, locale);
  return {
    id: exerciseId,
